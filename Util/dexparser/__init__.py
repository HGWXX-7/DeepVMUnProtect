...
from automaticvmcracker.Util.dexparser import disassembler
from automaticvmcracker.Util.dexparser.utils import uleb128_value, encoded_field, encoded_method, encoded_annotation
from automaticvmcracker.Util.dexparser import utils
from automaticvmcracker.Util.dexparser.errors import InsufficientParameterError

# from ..dexparser import disassembler
# from .utils import uleb128_value, encoded_field, encoded_method, encoded_annotation
# from .errors import InsufficientParameterError
# from . import utils
import struct
import mmap
...
class Dexparser(object):
    ...
    def get_string_by_idx(self, string_index):
        ...
        #result = ""
        #try:
        #    result = c_char.decode(encoding='utf-8')
        #except:
        #    print("Fail to decode, byte is {}".format(c_char))
        #    assert False, "quit here"
        return c_char.decode(encoding='utf-8', errors='ignore')
    ...
    def get_native_method(self):
        ...
        for i in range(self.header_data['class_defs_size']):
            ...
            # counter += 1
            # get next class data off
            next_class_data_off = -1
            if i != self.header_data['class_defs_size'] - 1:
                next_class_data_off = struct.unpack('<L', self.data[class_offset + ((i + 1) * 32)
                                                                    + 24:class_offset + ((i + 1) * 32) + 28])[0]
            ...
            for method in direct_methods:
                ...
                if 'native' in access_flags:
                    ...
                    self.native_class.append("class_data_off: " + str(class_data_off) +
                                             " " + class_name + " " + str(next_class_data_off))
        ...
        return self.native_method, self.native_class

    def search_by_closest_debugoff(self, left_limit, right_limit, debugoff, direction):
        while True:
            if direction == 'plus':
                debugoff += 1
            else:
            ...
            try:
                ...
                if index != -1:
                    if index < right_limit and index > left_limit:
                        print("index is {}, debugoff is {}, direction is {}".format(index, debugoff, direction))
                        return index
            except ValueError:
                continue
    ...
    def get_debug_info_from_begin(self, methodname_to_debugoff, native_function_name):
        ...
        left_limit_offset_debug_info = 0
        right_limit_offset_debug_info = 0
        for item in methodname_to_debugoff:
            offset_debug_info = item['offset_of_debug_off']
            ...
            if methodname == native_function_name:
                ...
                left_limit_offset_debug_info = offset_debug_info
        ...
        for item in methodname_to_debugoff[::-1]:
            ...
            if methodname == native_function_name:
                ...
                right_limit_offset_debug_info = offset_debug_info
        print("[FROM_START] left:{}, right:{}".format(left_limit_offset_debug_info, right_limit_offset_debug_info))
        if closest_method_debug_off == 0 or right_limit_offset_debug_info == 0 or left_limit_offset_debug_info == 0:
            return None
        else:
            return self.search_by_closest_debugoff(left_limit_offset_debug_info, right_limit_offset_debug_info,
                                                   closest_method_debug_off, "plus")
    ...
    def get_debug_info_from_end(self, methodname_to_debugoff, native_function_name):
        ...
        print("[FROM_END] left:{}, right:{}".format(left_limit_offset_debug_info, right_limit_offset_debug_info))
        if closest_method_debug_off == 0 or right_limit_offset_debug_info == 0 or left_limit_offset_debug_info == 0:
            return None
        else:
            return self.search_by_closest_debugoff(left_limit_offset_debug_info, right_limit_offset_debug_info,
                                                   closest_method_debug_off, "minus")
    ...
    def get_debug_info_in_class(self, class_data_off):
        ...
        for method in direct_methods:
            ...
            offset_of_debug_off = method['code_off'] + 8
            debug_off = 0
            ...
            result.append({
                ...
                'debug_off': debug_off,
                'offset_of_debug_off': offset_of_debug_off,
            })
    ...
    def get_code_item_in_class(self, class_data_off):
        class_data_content = self.get_class_data(class_data_off)
        direct_methods = class_data_content['direct_methods']
        virtual_methods = class_data_content['virtual_methods']
        result = []
        ...
        for method in direct_methods:
            method_name = self.get_method_name_by_idx(method['diff'])
            code_item_off = method['code_off']
            instruction_length = struct.unpack("<L", self.data[code_item_off + 12: code_item_off + 16])[0]
            temp = code_item_off + 16 + instruction_length * 2
            # align to 4
            next_code_item_off = temp if temp % 4 == 0 else temp + 4 - temp % 4

            result.append({
                'method_name': method_name.strip(),
                'next_code_item_off': next_code_item_off,
            })
    ...
    def get_prev_code_item(self, methodname_to_debugoff, target_method_name):
        target_method_offset = 0
        for item in methodname_to_debugoff:
            method_name = item['method_name']
            next_code_item_off = item['next_code_item_off']
            if method_name == target_method_name:
                break

            target_method_offset = next_code_item_off

        assert target_method_offset != 0, "[DEXPARSER]: target method offset = 0, target method name:{}".format(target_method_name)


        return target_method_offset

    # def get_code_item(self, index: list):
    #     '''
    #     :param index: method index of target method
    #     :return:
    #     '''
    #     code_item_info = []
    #     for i in index:
    #         class_data_off = int(self.native_class[i].split(" ")[1].strip())
    #         methodname_to_code_item_off = self.get_code_item_in_class(class_data_off)
    #         target_method_name = self.native_method[i].split(" ")[-1].strip()
    #         code_item_offset = self.get_prev_code_item(methodname_to_code_item_off, target_method_name)
    #         first_bytecode_offset = code_item_offset + 16
    #         code_item_length = struct.unpack("<L", self.data[code_item_offset+12: code_item_offset+16])[0]
    #
    #         code_item_info.append({
    #             "length": code_item_length,
    #             "offset": first_bytecode_offset,
    #         })
    #
    #     # input("code item info get successfully")
    #     return code_item_info

    def get_debug_info_offset(self, index: list):
        debug_info_offset = []
        for i in index:
            class_data_off = int(self.native_class[i].split(" ")[1].strip())
            next_class_data_off = int(self.native_class[i].split(" ")[-1].strip())
            print(next_class_data_off)
            methodname_to_debugoff = self.get_debug_info_in_class(class_data_off)
            methodname_to_debugoff += self.get_debug_info_in_class(next_class_data_off)
            target_function_name = self.native_method[i].split(" ")[-1].strip()
            print("now we will process {} of class off {}".format(target_function_name, class_data_off))
            print("methodname_to_debugoff:\n{}".format(methodname_to_debugoff))
        ...
        # input("debug info get successfully")
